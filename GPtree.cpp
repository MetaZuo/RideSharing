...
const bool Optimization_Euclidean_Cut = false;//是否开启cache查询中基于欧几里得距离剪枝算法
const char Edge_File[] = "cal.edge";//第一行两个整数n,m表示点数和边数，接下来m行每行三个整数U,V,C表示U->V有一条长度为C的边
const char Node_File[] = "COL.co"; //共N行每行一个整数两个实数id,x,y表示id结点的经纬度(但输入不考虑id，只顺序从0读到n-1，整数N在Edge文件里)
...
struct G_Tree
{
	...
	struct Node
	{
		...
		int cache_id, cache_bound;//当前cache所保存的起点编号，当前cache所保存的已更新的cache的bound(<bound的begin已更新end)
		vector<int>cache_dist;//当前cache_dist保存的是从图G中结点cache_id到每个border的距离，其中只有值小于等于cache_bound的部分值是正确的
		vector<int>border_son_id;//当前border所在的儿子结点的编号
		int min_border_dist;//当前结点随cache缓存的边界点最小的距离(用于KNN剪枝)
	...
	void push_borders_up_cache(int x, int bound = INF)//将S到结点x边界点缓存在x.cache_dist的最短路长度，计算S到x.father真实border的距离更新x.father.cache
	{
		if (node[x].father == 0)return;
		int y = node[x].father;
	...
	void push_borders_down_cache(int x, int y, int bound = INF)//将S到结点x边界点缓存在x.cache_dist的最短路长度，计算S到x的儿子y真实border的距离更新y.cache
	{
		if (node[x].cache_id == node[y].cache_id&&bound <= node[y].cache_bound)return;
		node[y].cache_id = node[x].cache_id;
		node[y].cache_bound = bound;
	...
	void push_borders_brother_cache(int x, int y, int bound = INF)//将S到结点x边界点缓存在x.cache_dist的最短路长度，计算S到x的兄弟结点y真实border的距离更新y.cache
	{
	...
	int search_cache(int S, int T, int bound = INF)//查询S-T最短路长度,并将沿途结点的cache处理为S的结果，其中不计算权值>=bound的部分，若没有则剪枝返回INF
	{
		//朴素G-Tree计算,维护cache
		if (S == T)return 0;
		//计算LCA
